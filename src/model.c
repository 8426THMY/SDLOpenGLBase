...
typedef struct vertex {
	...
	size_t boneIDs[MAX_VERTEX_WEIGHTS];
	float boneWeights[MAX_VERTEX_WEIGHTS];
} vertex;


//Forward-declare any helper functions!
static unsigned char vertexUnique(const vertex *v1, const vertex *v2);
...
size_t modelLoad(const char *mdlName){
	size_t index = loadedModels.size;
	...
	char *mdlPath = malloc(18 + mdlNameLength + 1);
	memcpy(mdlPath, ".\\resource\\models\\", 18);
	strcpy(mdlPath + 18, mdlName);
	...
	if(mdlFile != NULL){
		...
		//Temporarily stores vertex positions, regardless of whether or not it is unique.
		size_t tempPositionsSize = 0;
		size_t tempPositionsCapacity = BASE_VERTEX_CAPACITY;
		vec3 *tempPositions = malloc(tempPositionsCapacity * sizeof(*tempPositions));
		//Temporarily stores vertex UVs, regardless of whether or not it is unique.
		size_t tempUVsSize = 0;
		size_t tempUVsCapacity = BASE_VERTEX_CAPACITY * 2;
		float *tempUVs = malloc(tempUVsCapacity * sizeof(*tempUVs));
		//Temporarily stores vertex normals, regardless of whether or not it is unique.
		size_t tempNormalsSize = 0;
		size_t tempNormalsCapacity = BASE_VERTEX_CAPACITY;
		vec3 *tempNormals = malloc(tempNormalsCapacity * sizeof(*tempNormals));
		...
		while((line = readLineFile(mdlFile, &lineBuffer[0], &lineLength)) != NULL && index != -1){
			//Vertex positions.
			if(memcmp(line, "v ", 2) == 0){
				...
				if(tempPositionsSize >= tempPositionsCapacity){
					...
					tempPositions = realloc(tempPositions, tempPositionsCapacity * sizeof(*tempPositions));
				}
			...
			//Vertex UVs.
				...
				if(tempUVsSize >= tempUVsCapacity){
					...
					tempUVs = realloc(tempUVs, tempUVsCapacity * sizeof(*tempUVs));
				}
			...
			//Vertex normals.
				...
				if(tempNormalsSize >= tempNormalsCapacity){
					...
					tempNormals = realloc(tempNormals, tempNormalsCapacity * sizeof(*tempNormals));
				}
			...
			//TextureGroup path.
				...
				//We don't want to be keeping textureGroups that aren't used, so we'll load this
				//once we can be sure everything else was successful.
				tempTexGroupName = malloc((lineLength - 6) * sizeof(*tempTexGroupName));
			...
			//Faces.
				...
				for(a = 0; a < 3 || curTok != NULL; ++a){
					...
					memset(&tempVertex.boneIDs[1], -1, (MAX_VERTEX_WEIGHTS - 1) * sizeof(tempVertex.boneIDs[0]));
					tempVertex.boneWeights[0] = 1.f;
					memset(&tempVertex.boneWeights[1], 0.f, (MAX_VERTEX_WEIGHTS - 1) * sizeof(tempVertex.boneWeights[0]));
					...
					if(a >= 3){
						...
						tempIndices[mdl.totalIndices - 2] = tempIndices[mdl.totalIndices - 2 - a];
						//Now get the index of the last vertex that this 'f' statement used.
						tempIndices[mdl.totalIndices - 1] = tempIndices[mdl.totalIndices - 3];
					...
					tempIndices[mdl.totalIndices] = b;
					++mdl.totalIndices;
			...
			//Syntax error.
				...
				printf("Syntax error loading model!\n"
				       "Path: %s\n"
				       "Model ID: %u\n"
				       "Line: %s\n", mdlPath, index, line);
				index = -1;
		...
		if(index != -1){
			...
			glBindVertexArray(mdl.vertexArrayID);
				...
				glVertexAttribIPointer(3, MAX_VERTEX_WEIGHTS, GL_UNSIGNED_INT, sizeof(vertex), (GLvoid *)offsetof(vertex, boneIDs));
				glVertexAttribPointer(4, MAX_VERTEX_WEIGHTS, GL_FLOAT, GL_FALSE, sizeof(vertex), (GLvoid *)offsetof(vertex, boneWeights));
					...
					if(dataType == 1){
						...
						if(boneID == tempBonesSize){
							...
							printf("Error loading model!\n"
							       ...
							       mdlPath, index, line, boneID, tempBonesSize);
							index = -1;
						}
					}else if(dataType == 2){
						...
						if(data == 0){
							...
							if(parentBoneID < tempBonesSize){
								...
								if(numLinks > 0){
									...
									if(numLinks > MAX_VERTEX_WEIGHTS){
										printf("Error loading model!\n"
										       "Path: %s\n"
										       "Model ID: %u\n"
										       "Line: %s\n"
									...
									memset(&tempVertex.boneIDs[i], -1, (MAX_VERTEX_WEIGHTS - i) * sizeof(tempVertex.boneIDs[0]));
									memset(&tempVertex.boneWeights[i], 0.f, (MAX_VERTEX_WEIGHTS - i) * sizeof(tempVertex.boneWeights[0]));
...
unsigned char modelModuleSetup(){
	...
	if(loadedTextureGroups.size > 0){
		vectorInit(&loadedModels, sizeof(model));


		//Create and initialize an error model!
		model mdl;
		modelInit(&mdl);

		mdl.name = malloc(6);
		strcpy(mdl.name, "error");
		...
		glBindVertexArray(mdl.vertexArrayID);
			...
			glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, mdl.indexBufferID);
			//Now add all our data to it!
			glBufferData(GL_ELEMENT_ARRAY_BUFFER, (mdl.totalIndices * sizeof(*indices)), &indices[0], GL_STATIC_DRAW);
