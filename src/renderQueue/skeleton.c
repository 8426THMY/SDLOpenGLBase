...
static bone defaultBone = {
	...
	.invGlobalBind = &g_mat3x4Identity,

	.parent = SKELETON_INVALID_BONE_INDEX
};
...
void boneInit(
	...
	if(state != NULL){
		bone->invGlobalBind = *invGlobalBind;
...
void skeleInitSet(
	...
	if(bones != NULL){
		...
		for(; curBone < lastBone; ++curBone){
			mat3x4Invert(&curBone->invGlobalBind);
		}
...
void skeleAnimInit(
	...
	{
		const bone *curSkeleBone = skele->bones;
		const bone *const lastSkeleBone = &curSkeleBone[skele->numBones];
		boneIndex *curLookupID = anim->lookup;
		// Initialize the bone lookup.
		for(; curSkeleBone != lastSkeleBone; ++curSkeleBone){
			*curLookupID = skeleAnimDefFindBone(animDef, curSkeleBone->name);
			++curLookupID;
		}
	}
...
void skeleStateInit(skeletonState *const restrict skeleState, const skeleton *const restrict skele){
	...
	skeleState->states = memoryManagerGlobalAlloc(sizeof(*skeleState->states) * skele->numBones);
	if(skeleState->states == NULL){
		/** MALLOC FAILED **/
	}
	skeleState->prevStates = memoryManagerGlobalAlloc(sizeof(*skeleState->prevStates) * skele->numBones);
	if(skeleState->prevStates == NULL){
		/** MALLOC FAILED **/
	}
	skeleState->interpStates = memoryManagerGlobalAlloc(sizeof(*skeleState->interpStates) * skele->numBones);
	if(skeleState->interpStates == NULL){
		/** MALLOC FAILED **/
	}
}
...
void skeleStatePrependAnimations(
	...
	while(curAnim != NULL){
		const boneIndex animBoneID = curAnim->lookup[boneID];
		// Make sure this bone exists in the animation!
		#warning "This means that we can't have a bone with index -1.
		if(animBoneID != SKELETON_INVALID_BONE_INDEX){
			...
			#warning "It would be nice if we could handle additive (or delta) animations the same as normal ones."
			#warning "I think Source just subtracts the skeleton's local bind state from them when loading them."
...
void skeleStateDelete(skeletonState *const restrict skeleState){
	...
	if(skeleState->states != NULL){
		memoryManagerGlobalFree(skeleState->states);
	}
	if(skeleState->prevStates != NULL){
		memoryManagerGlobalFree(skeleState->prevStates);
	}
	if(skeleState->interpStates != NULL){
		memoryManagerGlobalFree(skeleState->interpStates);
	}
}