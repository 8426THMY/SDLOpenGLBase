...
// This is the ID of the currently active texture unit.
unsigned int curTextureUnit = 0;
// These are the ID's of the textures that are currently bound.
GLuint curTextureID[TEXTURE_NUM_UNITS] = {
	TEXTURE_INVALID_ID
};
...
static void setFiltering(
	const GLuint id, GLint filtering, const uint_least8_t mips
);
...
texture *textureLoad(const char *const restrict texPath, const size_t texPathLength){
	...
	if(texFile != NULL){
		...
		if(image != NULL){
			...
			setFiltering(texID, filtering, 0);
			...
			// If we could set up the texture successfully,
			// allocate memory for it and set it up!
			if(openGLError == GL_NO_ERROR){
				...
				curTextureID[curTextureUnit] = texID;
				tex->id = texID;
				tex->width  = image->w;
				tex->height = image->h;
				...
				// If something went wrong, we'll need
				// to delete the texture we just created.
...
void textureBind(
	...
	// Only bind the texture if it's not already bound.
	if(tex->id != curTextureID[unit]){
		// If it's not bound, we'll need to make sure that
		// the right texture unit is active before binding.
		if(unit != curTextureUnit){
			curTextureUnit = unit;
			glActiveTexture(GL_TEXTURE0 + curTextureUnit);
		}
		curTextureID[unit] = tex->id;
		glBindTexture(GL_TEXTURE_2D, curTextureID[unit]);
	}
}
...
void textureDelete(texture *const restrict tex){
	...
	{
		GLuint *curID = curTextureID;
		const GLuint *const lastID = curID[TEXTURE_NUM_UNIT];
		// If the texture we're deleting was bound to any
		// texture units, we'll need to fix up our state.
		for(; curID != lastID; ++curID){
			if(*curID == tex->id){
				*curID = TEXTURE_INVALID_ID;
			}
		}
	}
}
