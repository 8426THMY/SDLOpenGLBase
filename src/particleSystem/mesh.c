...
void meshDrawInstanced(const mesh *const restrict meshData, const GLsizei numInstances){
	glBindVertexArray(meshData->vertexArrayID);
	// We probably should either unmap buffers or upload instance data here.
...
return_t meshSetup(){
	// We should allocate the global instance data buffer here.
	return(1);
}

void meshCleanup(){
	// We should delete the global instance data buffer here.
}