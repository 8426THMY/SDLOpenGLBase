...
void manifoldPresolve(contactManifold *cm){
	...
	const float invMass = cm->bodyA->body->invMass + cm->bodyB->body->invMass;
	mat3 *invInertiaA = &cm->bodyA->invInertia;
	mat3 *invInertiaB = &cm->bodyB->invInertia;

	contactPoint *curContact = cm->contacts;
	contactPoint *lastContact = &cm->contacts[cm->numContacts];
	...
	for(; curContact < lastContact; ++curContact){
		...
		vec3CrossVec3(&curContact->rA, &cm->normal, &rAX);
		vec3CrossVec3(&curContact->rB, &cm->normal, &rBX);
		...
		vec3CrossVec3(&cm->tangents[0], &curContact->rA, &rAX);
		vec3CrossVec3(&cm->tangents[0], &curContact->rB, &rBX);
		mat3MultiplyVec3R(invInertiaA, &rAX, &rAXI);
		mat3MultiplyVec3R(invInertiaB, &rBX, &rBXI);
		//1 / (mA + mB + ((rA X t0) . (IA * (rA X t0))) + ((rB X t0) . (IB * (rB X t0))))
		...
		vec3CrossVec3(&cm->tangents[1], &curContact->rA, &rAX);
		vec3CrossVec3(&cm->tangents[1], &curContact->rB, &rBX);
		mat3MultiplyVec3R(invInertiaA, &rAX, &rAXI);
		mat3MultiplyVec3R(invInertiaB, &rBX, &rBXI);
		//1 / (mA + mB + ((rA X t1) . (IA * (rA X t1))) + ((rB X t1) . (IB * (rB X t1))))
